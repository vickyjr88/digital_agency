...
TABLE_COLUMNS = [
    ...
    ("deliverables", "platform", "platformtypedb"),
    ("packages", "status", "packagestatusdb"),
    ("packages", "platform", "platformtypedb"),
...
def sync_marketplace():
    print(f"🚀 Starting Marketplace Sync on {DATABASE_URL.split('@')[-1]}")
    
    # 1. Add missing columns to deliverables if needed (Marketplace Refactor)
    with engine.begin() as conn:
        print("🔍 Checking 'deliverables' columns...")
        for col_name in ["bid_id", "influencer_id"]:
            res = conn.execute(text(f"SELECT column_name FROM information_schema.columns WHERE table_name='deliverables' AND column_name='{col_name}';")).fetchone()
            if not res:
                print(f"➕ Adding '{col_name}' to 'deliverables'...")
                conn.execute(text(f"ALTER TABLE deliverables ADD COLUMN {col_name} VARCHAR(36);"))

    # 2. Ensure all lowercase values exist in ENUM types
    # Postgres ALTER TYPE ... ADD VALUE cannot run in a transaction block
    with engine.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
        for type_name, values in ENUM_TYPES:
            print(f"🔍 Checking enum type: {type_name}")
            
            # Check if type exists
            type_exists = conn.execute(text(f"SELECT EXISTS (SELECT 1 FROM pg_type WHERE typname = '{type_name}')")).scalar()
            if not type_exists:
                print(f"⏩ Enum type {type_name} does not exist, skipping labels check.")
                continue
    ...
    # 3. Update existing data to lowercase
    with engine.begin() as conn:
    ...
    print("🎉 Marketplace synchronization complete!")
