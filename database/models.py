...
class User(Base):
    ...
    role = Column(Enum(UserRole, values_callable=lambda x: [e.value for e in x], native_enum=False, length=20), default=UserRole.USER)
    user_type = Column(Enum(UserType, values_callable=lambda x: [e.value for e in x], native_enum=False, length=20), default=UserType.BRAND)  # Marketplace user type
    subscription_tier = Column(Enum(SubscriptionTier, values_callable=lambda x: [e.value for e in x], native_enum=False, length=20), default=SubscriptionTier.FREE)
    subscription_status = Column(Enum(SubscriptionStatus, values_callable=lambda x: [e.value for e in x], native_enum=False, length=20), default=SubscriptionStatus.TRIAL)
...
class Content(Base):
    ...
    status = Column(Enum(ContentStatus, values_callable=lambda x: [e.value for e in x], native_enum=False, length=20), default=ContentStatus.PENDING)
...
class TeamMember(Base):
    ...
    role = Column(Enum(TeamRole, values_callable=lambda x: [e.value for e in x], native_enum=False, length=20), default=TeamRole.VIEWER)
...
class Transaction(Base):
    ...
    status = Column(Enum(PaymentStatus, values_callable=lambda x: [e.value for e in x], native_enum=False, length=20), default=PaymentStatus.PENDING)
